...
from imly.views.stores import OrderList, StoreList, StoreCreate, StoreDetail, StoreEdit, StoreInfoDetail, home_page, why_open_your_shop, contact_us, faqs, what_is_imly, wrong_location, status, update_store_order, no_city, not_in_city, one_step_checkout,upload_images
from imly.views.products import SpecialList, ProductReview, ProductList, ProductCreate, ProductDelete, ProductDetail, ProductEdit, ProductsByAccount, coming_soon,sort_product,activate_product,special_event,unsubscribe_event,wish_product,wishlist,remove_product,upload_product_images
...
urlpatterns = patterns('',
    ...
    url(r"^shops/$", StoreList.as_view(), name="imly_store_list"),
    url(r"^chefs/$", ProfileList.as_view(), name="imly_profiles"),
    url(r"^chefs/(?P<slug>[-\w]+)/$", ChefProfile.as_view(), name="chef_profile"),
