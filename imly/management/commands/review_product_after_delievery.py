...
class Command(BaseCommand):
	def handle(self,*args,**options):
		...
		for sto in StoreOrder.objects.filter(order__status = Order.IMLY_CONFIRMED, delivered_on__startswith=date.today() - timedelta(days=1)):
			msg = EmailMessage("Review purchase product",get_template('email_templates/review_product_after_delivery.html').render(Context({'storeorder':sto,'site':site})),settings.ADMIN_EMAIL,[sto.order.email])
